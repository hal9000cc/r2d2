...
class TestBuySltpOneEntryOneTake:
    ...
    def test_buy_sltp_limit_entry_take_simultaneous(self, test_task):
        ...
        assert collected_data[2]['trades_count'] == 2, "Entry should trigger on bar 1 and take profit on bar 2 (both visible after bar 2 processing)"
...
class TestBuySltpOneEntryMultipleTakes:
    ...
    def test_buy_sltp_limit_entry_all_takes_simultaneous(self, test_task):
        ...
        assert collected_data[0]['trades_count'] == 0, "No execution on bar 0"
        assert collected_data[2]['trades_count'] == 3, "Entry should trigger on bar 1 and both take profits on bar 2 (all visible after bar 2 processing)"
    ...
    def test_buy_sltp_limit_entry_part_takes_simultaneous(self, test_task):
        ...
        # Third take (extreme, gets remainder): 1.0 - 0.7 = 0.3 (but doesn't trigger)
        # First two takes trigger, closing 0.3 + 0.4 = 0.7 of 1.0
        take_quantity1 = 0.3  # First take: 0.33 rounded to 0.3
        take_quantity2 = 0.4  # Second take: 0.36 rounded to 0.4 (accumulated error from first)
        remaining_quantity = quantity - take_quantity1 - take_quantity2  # 1.0 - 0.3 - 0.4 = 0.3
        ...
        assert collected_data[0]['trades_count'] == 0, "No execution on bar 0"
        assert collected_data[2]['trades_count'] == 3, "Entry should trigger on bar 1 and first two take profits on bar 2 (all visible after bar 2 processing)"
...
# ============================================================================
# Group D2: One Entry, Multiple Take Profits - SELL
# ============================================================================

class TestSellSltpOneEntryMultipleTakes:
    """Test D2: One entry, multiple take profits scenarios for sell_sltp."""
    
    def test_sell_sltp_limit_entry_all_takes_simultaneous(self, test_task):
        """Test D2.1: Limit entry and all take profits hit on same bar → entry triggers on bar 1, all takes trigger on bar 2."""
        # Prepare quotes data: price 100.0, then falls to trigger limit entry on bar 1, all take profits on bar 2
        # Bar 0: high=101.0, low=99.0, limit=105.0, takes at 90.0 and 88.0 - won't trigger (101.0 < 105.0 for limit, 99.0 > 90.0 for takes)
        # Bar 1: high=106.0, low=87.0, limit=105.0, takes at 90.0 and 88.0 - entry triggers (106.0 >= 105.0), takes activate but don't trigger yet
        #   Entry limit (SELL, triggers when high >= price): 106.0 >= 105.0 ✓
        #   Take profits (BUY limits, trigger when low <= price): 87.0 <= 90.0 ✓, 87.0 <= 88.0 ✓, but takes are NEW, activate after entry executes
        # Bar 2: low=87.0, takes at 90.0 and 88.0 - both take profits trigger (87.0 <= 90.0, 87.0 <= 88.0)
        quotes_data = create_custom_quotes_data(
            prices=[100.0, 97.0, 87.0],
            highs=[101.0, 106.0, 92.0],  # Bar 1 high=106.0 triggers entry limit at 105.0 (106.0 >= 105.0)
            lows=[99.0, 87.0, 87.0]  # Bar 1 low=87.0 (entry triggers), Bar 2 low=87.0 triggers both take profits at 90.0 and 88.0 (87.0 <= 90.0, 87.0 <= 88.0)
        )
        
        # Protocol: On bar 0, enter limit at 105.0 with stop loss 110.0 and two take profits (0.5 at 90.0, 0.5 at 88.0)
        # Entry price: 105.0 (limit, no slippage, fee_maker)
        ...
        exit_fee1 = exit_execution1 * take_quantity1 * test_task.fee_maker  # 90.0 * 0.5 * 0.0005 = 0.0225
        exit_execution2 = take_price2  # 88.0 (limit, no slippage)
        exit_fee2 = exit_execution2 * take_quantity2 * test_task.fee_maker  # 88.0 * 0.5 * 0.0005 = 0.022
        
        entry_proceeds = entry_execution * quantity - entry_fee  # 105.0*1.0 - 0.0525 = 104.9475
        total_exit_cost = exit_execution1 * take_quantity1 + exit_fee1 + exit_execution2 * take_quantity2 + exit_fee2  # 90.0*0.5 + 0.0225 + 88.0*0.5 + 0.022 = 89.0445
        expected_profit = entry_proceeds - total_exit_cost  # = 104.9475 - 89.0445 = 15.903
        ...
        protocol = [
            {
                ...
                'args': {
                    'enter': (1.0, 105.0),  # Limit order at 105.0
                    'stop_loss': 110.0,
                    'take_profit': [(0.5, 90.0), (0.5, 88.0)]  # Two take profits with equal shares
                }
            }
        ]
        ...
        with patch('app.services.tasks.broker_backtesting.QuotesClient') as mock_client_class:
            ...
            test_task.isRunning = True
            with patch('app.services.tasks.tasks.Task.load', return_value=test_task):
                broker, strategy = create_broker_and_strategy(test_task, quotes_data, "test_sell_d2_1_limit_entry_all_takes_simultaneous")
                broker.run(save_results=False)
    ...
    def test_sell_sltp_limit_entry_part_takes_simultaneous(self, test_task):
        """Test D2.2: Limit entry and part of take profits hit on same bar → entry triggers on bar 1, part of takes trigger on bar 2."""
        # Prepare quotes data: price 100.0, then falls to trigger limit entry on bar 1, part of take profits on bar 2
        # Bar 0: high=101.0, low=99.0, limit=105.0, takes at 90.0, 88.0, 86.0 - won't trigger (101.0 < 105.0 for limit, 99.0 > 90.0 for takes)
        # Bar 1: high=106.0, low=87.0, limit=105.0, takes at 90.0, 88.0, 86.0 - entry triggers (106.0 >= 105.0), takes activate but don't trigger yet
        #   Entry limit (SELL, triggers when high >= price): 106.0 >= 105.0 ✓
        ...
        quotes_data = create_custom_quotes_data(
            prices=[100.0, 97.0, 87.0],
            highs=[101.0, 106.0, 92.0],  # Bar 1 high=106.0 triggers entry limit at 105.0 (106.0 >= 105.0)
            lows=[99.0, 87.0, 87.0]  # Bar 1 low=87.0 (entry triggers), Bar 2 low=87.0 triggers first two take profits at 90.0 and 88.0 (87.0 <= 90.0, 87.0 <= 88.0) but not third at 86.0 (87.0 > 86.0)
        )
        
        # Protocol: On bar 0, enter limit at 105.0 with stop loss 110.0 and three take profits (0.33 at 90.0, 0.33 at 88.0, 0.34 at 86.0)
        # Entry price: 105.0 (limit, no slippage, fee_maker)
        # Take triggers: 90.0 and 88.0 (takes execute as limits, no slippage, fee_maker)
        # Expected: limit entry triggers on bar 1, first two take profits activate and trigger on bar 2, third take remains active
        # Expected profit calculation (with volume rounding to precision_amount=0.1):
        ...
        exit_execution2 = take_price2  # 88.0 (limit, no slippage)
        exit_fee2 = exit_execution2 * take_quantity2 * test_task.fee_maker  # 88.0 * 0.4 * 0.0005 = 0.0176
        
        # Remaining position (0.3) will be auto-closed at end of test at bar 2 closing price (87.0)
        ...
        auto_close_fee = auto_close_execution * remaining_quantity * test_task.fee_taker  # 87.1 * 0.3 * 0.001 = 0.02613
        
        entry_proceeds = entry_execution * quantity - entry_fee  # 105.0*1.0 - 0.0525 = 104.9475
        total_exit_cost = exit_execution1 * take_quantity1 + exit_fee1 + exit_execution2 * take_quantity2 + exit_fee2 + auto_close_execution * remaining_quantity + auto_close_fee  # 90.0*0.3 + 0.0135 + 88.0*0.4 + 0.0176 + 87.1*0.3 + 0.02613 = 27.0135 + 35.2176 + 26.15613 = 88.38723
        expected_profit = entry_proceeds - total_exit_cost  # = 104.9475 - 88.38723 = 16.56027
        ...
        protocol = [
            {
                ...
                'args': {
                    'enter': (1.0, 105.0),  # Limit order at 105.0
                    'stop_loss': 110.0,
                    'take_profit': [(0.33, 90.0), (0.33, 88.0), (0.34, 86.0)]  # Three take profits with shares
                }
            }
        ]
        ...
        with patch('app.services.tasks.broker_backtesting.QuotesClient') as mock_client_class:
            ...
            test_task.isRunning = True
            with patch('app.services.tasks.tasks.Task.load', return_value=test_task):
                broker, strategy = create_broker_and_strategy(test_task, quotes_data, "test_sell_d2_2_limit_entry_part_takes_simultaneous")
                broker.run(save_results=False)
...
# ============================================================================
# Group D3: Multiple Entries, All Take Profits - BUY
# ============================================================================

class TestBuySltpMultipleEntriesAllTakes:
    """Test D3: Multiple entries, all take profits scenarios for buy_sltp."""
    
    def test_buy_sltp_multiple_limits_all_takes_simultaneous(self, test_task):
        """Test D3.1: Multiple limit entries and all take profits hit on same bar → entries trigger on bar 1, all takes trigger on bar 2."""
        # Prepare quotes data: price 100.0, then rises to trigger limit entries on bar 1, all take profits on bar 2
        # Bar 0: high=101.0, low=99.0, limits at 97.0 and 95.0, takes at 110.0 and 112.0 - won't trigger (99.0 > 97.0, 99.0 > 95.0, 101.0 < 110.0, 101.0 < 112.0)
        # Bar 1: high=113.0, low=94.0, limits at 97.0 and 95.0, takes at 110.0 and 112.0 - entries trigger (94.0 <= 97.0, 94.0 <= 95.0), takes activate but don't trigger yet
        #   Entry limits (BUY, triggers when low <= price): 94.0 <= 97.0 ✓, 94.0 <= 95.0 ✓
        #   Take profits (SELL limits, trigger when high >= price): 113.0 >= 110.0 ✓, 113.0 >= 112.0 ✓, but takes are NEW, activate after entry executes
        # Bar 2: high=113.0, takes at 110.0 and 112.0 - both take profits trigger (113.0 >= 110.0, 113.0 >= 112.0)
        quotes_data = create_custom_quotes_data(
            prices=[100.0, 103.0, 113.0],
            highs=[101.0, 113.0, 113.0],  # Bar 1 high=113.0 (entries trigger), Bar 2 high=113.0 triggers both take profits at 110.0 and 112.0 (113.0 >= 110.0, 113.0 >= 112.0)
            lows=[99.0, 94.0, 112.0]  # Bar 1 low=94.0 triggers both limit entries at 97.0 and 95.0 (94.0 <= 97.0, 94.0 <= 95.0)
        )
        
        # Protocol: On bar 0, enter with two limits (0.5 at 97.0, 0.5 at 95.0) with two take profits (0.5 at 110.0, 0.5 at 112.0) and stop loss 90.0
        # Entry prices: 97.0 and 95.0 (limits, no slippage, fee_maker)
        # Take triggers: 110.0 and 112.0 (takes execute as limits, no slippage, fee_maker)
        # Expected: both limit entries trigger on bar 1, both take profits activate and trigger on bar 2
        # Expected profit calculation:
        entry_price1 = 97.0
        entry_price2 = 95.0
        quantity = 1.0
        ...
        take_price1 = 110.0
        take_price2 = 112.0
        take_quantity1 = 0.5  # First take
        take_quantity2 = 0.5  # Second take
        ...
        total_exit_proceeds = exit_execution1 * take_quantity1 - exit_fee1 + exit_execution2 * take_quantity2 - exit_fee2  # 110.0*0.5 - 0.0275 + 112.0*0.5 - 0.028 = 110.9445
        expected_profit = total_exit_proceeds - entry_cost  # = 110.9445 - 96.048 = 14.8965
        ...
        with patch('app.services.tasks.broker_backtesting.QuotesClient') as mock_client_class:
            ...
            test_task.isRunning = True
            with patch('app.services.tasks.tasks.Task.load', return_value=test_task):
                broker, strategy = create_broker_and_strategy(test_task, quotes_data, "test_buy_d3_1_multiple_limits_all_takes_simultaneous")
                broker.run(save_results=False)
        ...
        # Check that entries trigger on bar 1, both take profits trigger on bar 2
        # Bar 0: no execution (0 trades)
        ...
        assert collected_data[0]['trades_count'] == 0, "No execution on bar 0"
        assert collected_data[2]['trades_count'] == 4, "Both entries should trigger on bar 1 and both take profits on bar 2 (all visible after bar 2 processing)"
        ...
        # Check total trades count
        assert len(broker.trades) == 4, f"Expected 4 trades total (entry1 + entry2 + take1 + take2), got {len(broker.trades)}"
...
# ============================================================================
# Group D3: Multiple Entries, All Take Profits - SELL
# ============================================================================

class TestSellSltpMultipleEntriesAllTakes:
    """Test D3: Multiple entries, all take profits scenarios for sell_sltp."""
    
    def test_sell_sltp_multiple_limits_all_takes_simultaneous(self, test_task):
        """Test D3.1: Multiple limit entries and all take profits hit on same bar → entries trigger on bar 1, all takes trigger on bar 2."""
        # Prepare quotes data: price 100.0, then falls to trigger limit entries on bar 1, all take profits on bar 2
        # Bar 0: high=101.0, low=99.0, limits at 103.0 and 105.0, takes at 90.0 and 88.0 - won't trigger (101.0 < 103.0, 101.0 < 105.0, 99.0 > 90.0, 99.0 > 88.0)
        # Bar 1: high=106.0, low=87.0, limits at 103.0 and 105.0, takes at 90.0 and 88.0 - entries trigger (106.0 >= 103.0, 106.0 >= 105.0), takes activate but don't trigger yet
        ...
        quotes_data = create_custom_quotes_data(
            ...
            lows=[99.0, 87.0, 87.0]  # Bar 1 low=87.0 (entries trigger), Bar 2 low=87.0 triggers both take profits at 90.0 and 88.0 (87.0 <= 90.0, 87.0 <= 88.0)
        )
        
        # Protocol: On bar 0, enter with two limits (0.5 at 103.0, 0.5 at 105.0) with two take profits (0.5 at 90.0, 0.5 at 88.0) and stop loss 110.0
        ...
        total_exit_cost = exit_execution1 * take_quantity1 + exit_fee1 + exit_execution2 * take_quantity2 + exit_fee2  # 90.0*0.5 + 0.0225 + 88.0*0.5 + 0.022 = 89.0445
        expected_profit = entry_proceeds - total_exit_cost  # = 103.948 - 89.0445 = 14.9035
        ...
        with patch('app.services.tasks.broker_backtesting.QuotesClient') as mock_client_class:
            ...
            test_task.isRunning = True
            with patch('app.services.tasks.tasks.Task.load', return_value=test_task):
                broker, strategy = create_broker_and_strategy(test_task, quotes_data, "test_sell_d3_1_multiple_limits_all_takes_simultaneous")
                broker.run(save_results=False)
...
class TestBuySltpMultipleEntriesPartTakes:
    ...
    def test_buy_sltp_multiple_limits_all_entries_part_takes_simultaneous(self, test_task):
        ...
        # Only first two takes trigger, closing 0.3 + 0.4 = 0.7 of 1.0
        take_quantity1 = 0.3  # First take: 0.33 rounded to 0.3
        take_quantity2 = 0.4  # Second take: 0.36 rounded to 0.4 (accumulated error from first)
        remaining_quantity = total_entry_quantity - take_quantity1 - take_quantity2  # 1.0 - 0.3 - 0.4 = 0.3
        ...
        # First two takes close 0.7 of 1.0 (0.3 + 0.4)
        exit_execution1 = take_price1  # 110.0 (limit, no slippage)
        exit_fee1 = exit_execution1 * take_quantity1 * test_task.fee_maker  # 110.0 * 0.3 * 0.0005 = 0.0165
        exit_execution2 = take_price2  # 112.0 (limit, no slippage)
        exit_fee2 = exit_execution2 * take_quantity2 * test_task.fee_maker  # 112.0 * 0.4 * 0.0005 = 0.0224
        
        # Remaining position (0.3) will be auto-closed at end of test at bar 2 closing price (113.0)
        auto_close_price = 113.0  # Price close of last bar (bar 2)
        auto_close_execution = auto_close_price - test_task.slippage_in_steps * test_task.price_step  # 113.0 - 0.1 = 112.9 (SELL market, slippage decreases price)
        auto_close_fee = auto_close_execution * remaining_quantity * test_task.fee_taker  # 112.9 * 0.3 * 0.001 = 0.03387
        
        entry_cost = entry_execution1 * quantity1 + entry_fee1 + entry_execution2 * quantity2 + entry_fee2  # 97.0*0.5 + 0.02425 + 95.0*0.5 + 0.02375 = 96.048
        total_exit_proceeds = exit_execution1 * take_quantity1 - exit_fee1 + exit_execution2 * take_quantity2 - exit_fee2 + auto_close_execution * remaining_quantity - auto_close_fee  # 110.0*0.3 - 0.0165 + 112.0*0.4 - 0.0224 + 112.9*0.3 - 0.03387 = 32.9835 + 44.7776 + 33.83613 = 111.59723
        expected_profit = total_exit_proceeds - entry_cost  # = 111.59723 - 96.048 = 15.54923
        ...
        assert collected_data[0]['trades_count'] == 0, "No execution on bar 0"
        assert collected_data[2]['trades_count'] == 4, "Both entries should trigger on bar 1 and first two take profits on bar 2 (all visible after bar 2 processing)"
    ...
    def test_buy_sltp_multiple_limits_part_entries_part_takes_simultaneous(self, test_task):
        ...
        take_price1 = 110.0
        take_price2 = 112.0
        # Take profit volumes are calculated from CURRENT market volume (quantity = 0.6), not from full entry volume
        # Using cumulative rounding algorithm:
        # Fractions: 0.33, 0.33, 0.34, fraction_sum = 1.0
        # Target volume = current quantity = 0.6
        # First take: exact=0.33*0.6/1.0=0.198, exact_sum=0.198, order_vol=0.198-0.0=0.198, rounded=0.2, rounded_sum=0.2
        # Second take: exact=0.33*0.6/1.0=0.198, exact_sum=0.396, order_vol=0.396-0.2=0.196, rounded=0.2, rounded_sum=0.4
        # Third take (extreme, gets remainder from target volume): 0.6 - 0.4 = 0.2, rounded=0.2 (but doesn't trigger)
        # Only first two takes trigger, closing 0.2 + 0.2 = 0.4 of actual entered volume (0.6)
        take_quantity1 = 0.2  # First take: 0.198 rounded to 0.2
        take_quantity2 = 0.2  # Second take: 0.196 rounded to 0.2
        ...
        auto_close_price = 113.0
        slippage = test_task.slippage_in_steps * test_task.price_step  # 1.0 * 0.1 = 0.1
        auto_close_execution = auto_close_price - slippage  # 113.0 - 0.1 = 112.9 (SELL market, slippage decreases price)
        ...
        assert collected_data[0]['trades_count'] == 0, "No execution on bar 0"
        assert collected_data[2]['trades_count'] == 4, "First two entries should trigger on bar 1 and first two take profits on bar 2 (all visible after bar 2 processing)"
...
class TestSellSltpMultipleEntriesPartTakes:
    ...
    def test_sell_sltp_multiple_limits_all_entries_part_takes_simultaneous(self, test_task):
        ...
        entry_proceeds = entry_execution1 * quantity1 - entry_fee1 + entry_execution2 * quantity2 - entry_fee2  # 103.0*0.5 - 0.02575 + 105.0*0.5 - 0.02625 = 103.948
        total_exit_cost = exit_execution1 * take_quantity1 + exit_fee1 + exit_execution2 * take_quantity2 + exit_fee2 + auto_close_execution * remaining_quantity + auto_close_fee  # 90.0*0.3 + 0.0135 + 88.0*0.4 + 0.0176 + 87.1*0.3 + 0.02613 = 27.0135 + 35.2176 + 26.15613 = 88.38723
        expected_profit = entry_proceeds - total_exit_cost  # = 103.948 - 88.38723 = 15.56077
    ...
    def test_sell_sltp_multiple_limits_part_entries_part_takes_simultaneous(self, test_task):
        """Test D4.2: Multiple limit entries and part of take profits hit on same bar → part of entries trigger on bar 1, part of takes trigger on bar 2."""
        # Prepare quotes data: price 100.0, then falls to trigger part of limit entries and take profits on bar 1, part of take profits on bar 2
        ...
        # Bar 1: high=106.0, low=89.0, limits at 103.0, 105.0, 107.0, takes at 90.0, 88.0, 86.0 - first two entries and first take trigger
        #   Entry limits (SELL, triggers when high >= price): 106.0 >= 103.0 ✓, 106.0 >= 105.0 ✓, 106.0 < 107.0 ✗
        #   Take profits (BUY limits, trigger when low <= price): 89.0 <= 90.0 ✓ (take1 triggers), 89.0 <= 88.0 ✗, 89.0 > 86.0 ✗
        # Bar 2: low=87.0, takes at 90.0, 88.0, 86.0 - second take profit triggers (87.0 <= 88.0), third doesn't trigger (87.0 > 86.0)
        quotes_data = create_custom_quotes_data(
            prices=[100.0, 97.0, 87.0],
            ...
            lows=[99.0, 89.0, 87.0]  # Bar 1 low=89.0 triggers first take profit at 90.0 (89.0 <= 90.0) but not second at 88.0 (89.0 > 88.0), Bar 2 low=87.0 triggers second take profit at 88.0 (87.0 <= 88.0) but not third at 86.0 (87.0 > 86.0)
        )
        ...
        # Take triggers: 90.0 on bar 1, 88.0 on bar 2 (takes execute as limits, no slippage, fee_maker) - first two trigger
        # Expected: first two limit entries and first take profit trigger on bar 1, second take profit triggers on bar 2, third entry and third take remain active
        # Expected profit calculation (with volume rounding to precision_amount=0.1):
        # Volume calculation rules:
        # 1. Entry volumes are rounded DOWN (floor_to_precision) independently for each order
        # 2. Take profit volumes are calculated from TOTAL actual rounded-down entry volumes (sum of all rounded entries), not from requested volumes
        ...
        auto_close_price = 87.0
        slippage = test_task.slippage_in_steps * test_task.price_step  # 1.0 * 0.1 = 0.1
        auto_close_execution = auto_close_price + slippage  # 87.0 + 0.1 = 87.1 (BUY market, slippage increases price)
        ...
        total_exit_cost = exit_execution1 * take_quantity1 + exit_fee1 + exit_execution2 * take_quantity2 + exit_fee2 + auto_close_execution * remaining_quantity + auto_close_fee  # 90.0*0.2 + 0.009 + 88.0*0.2 + 0.0088 + 87.1*0.2 + 0.01742 = 18.0 + 0.009 + 17.6 + 0.0088 + 17.42 + 0.01742 = 53.05522
