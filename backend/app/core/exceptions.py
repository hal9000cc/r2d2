class R2D2Exception(Exception):
    pass


class R2D2ExceptionBadTimeframeValue(R2D2Exception):
    def __init__(self, value):
        self.value = value
        super().__init__(f'Bad timeframe value: {value}')


class R2D2ExceptionEmptyBarData(R2D2Exception):
    def __init__(self):
        super().__init__('Empty bar data')


class R2D2ExceptionDataNotReceived(R2D2Exception):
