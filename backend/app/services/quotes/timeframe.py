...
from .exceptions import R2D2ExceptionBadTimeframeValue
    ...
    def cast(value):
        ...
        if type(value) == int:
            try:
                ...
                raise R2D2ExceptionBadTimeframeValue(value) from exception

        if type(value) == str:
            if hasattr(Timeframe, f't{value}'):
                return Timeframe[f't{value}']

        raise R2D2ExceptionBadTimeframeValue(value)
